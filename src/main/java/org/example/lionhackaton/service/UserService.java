...
public class UserService {
    ...
    public Long getPoint(CustomUserDetails customUserDetails){
        User user = userRepository.findById(customUserDetails.getId()).orElseThrow(() -> new NotFoundException("User not found"));
    ...
    public void minusDiaryPoint(CustomUserDetails customUserDetails, Diary diary){
        ...
        List<Diary> todayDiaries = user.getDiaries().stream().filter(diary1 -> diary1.getCreatedAt().toLocalDate().equals(today)).toList();
    ...
    public void minusCommentPoint(CustomUserDetails customUserDetails, Comment comment){
        User user = userRepository.findById(customUserDetails.getId()).orElseThrow(() -> new NotFoundException("User not found"));
        LocalDate today = LocalDate.now();
        List<Comment> todayComments = user.getComments().stream().filter(comments -> comments.getCreatedAt().toLocalDate().equals(today)).toList();
